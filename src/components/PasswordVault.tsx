...
const PasswordInput = ({ value, onChange, placeholder, show, onToggle, onGenerate }: any) => (
  <div style={{ position: 'relative', width: '100%' }}>
    <input
      ...
      onChange={onChange}
      required
      style={{ paddingRight: onGenerate ? '80px' : '45px', marginBottom: 0 }} 
    />
...
export default function PasswordVault({ onBack }: PasswordVaultProps) {
  ...
  // Começa carregando para esconder qualquer tela errada inicial
  ...
  const resetIdleTimer = () => {
    ...
    idleTimerRef.current = setTimeout(() => {
      ...
      setMasterPassword(''); 
      setShowForm(false);
      showToast('Bloqueado por inatividade!', 'error');
    }, 5 * 60 * 1000); 
  };
  ...
  const checkVaultStatus = async () => {
    ...
    // --- ATRASO ARTIFICIAL (O SEGREDO DO DESIGN SUAVE) ---
    // Forçamos o sistema a esperar pelo menos 800ms antes de mostrar a tela.
    // Isso evita o "flash" e mostra a animação de carregamento.
  ...
  const handleAuthAction = async (e: React.FormEvent) => {
    ...
    // Pequeno delay aqui também para dar feedback visual de que está "pensando"
    await new Promise(resolve => setTimeout(resolve, 500));
    
    if (isFirstAccess) {
      if (masterPassword.length < 4) { showToast('Senha muito curta!', 'error'); setLoading(false); return; }
      if (masterPassword !== confirmPassword) { showToast('As senhas não coincidem!', 'error'); setLoading(false); return; }
      showToast('Senha Mestra definida!', 'success');
      setIsUnlocked(true);
    } else {
      const { data } = await supabase.from('vault_items').select('*').order('created_at', { ascending: false });
      const vaultData = data || [];
      if (vaultData.length === 0) { setIsFirstAccess(true); setShowForm(true); setLoading(false); return; }
      
      const testItem = vaultData[0];
      const testDecryption = decryptData(testItem.password_encrypted, masterPassword);
      if (testDecryption) { setItems(vaultData); setIsUnlocked(true); } 
  ...
  const handleEdit = (item: VaultItem) => {
    const user = decryptData(item.username_encrypted, masterPassword);
    const pass = decryptData(item.password_encrypted, masterPassword);
    if (user && pass) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const userEncrypted = encryptData(newUser, masterPassword);
    const passEncrypted = encryptData(newPass, masterPassword);
    ...
    return (
      <div className="container" style={{paddingTop: '50px'}}> 
        <button 
            onClick={onBack} 
            style={{
                background: 'transparent', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 30, fontSize: '0.9rem', border: 'none', cursor: 'pointer'
            }}
        >
        ...
        <div className="card" style={{ textAlign: 'center', maxWidth: '400px', margin: '0 auto', minHeight: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          {loading ? (
            ...
            <>
              ...
              <h2>{isFirstAccess ? 'Configuração Inicial' : 'Cofre Bloqueado'}</h2>
              
              <p style={{color: '#94a3b8', marginBottom: '20px'}}>
                {isFirstAccess ? 'Defina sua Senha Mestra para começar.' : 'Digite sua Senha Mestra para acessar.'}
              </p>
              
              <form onSubmit={handleAuthAction}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '15px' }}>
                  <PasswordInput placeholder="Senha Mestra" value={masterPassword} onChange={(e: any) => setMasterPassword(e.target.value)} show={showMasterPass} onToggle={() => setShowMasterPass(!showMasterPass)} />
                  {isFirstAccess && (
                    <PasswordInput placeholder="Confirme a Senha" value={confirmPassword} onChange={(e: any) => setConfirmPassword(e.target.value)} show={showConfirmPass} onToggle={() => setShowConfirmPass(!showConfirmPass)} />
                  )}
                </div>
                <button type="submit" className="btn-primary" disabled={loading}>
                  {isFirstAccess ? 'Definir e Entrar' : 'Desbloquear'}
                </button>
              </form>
            </>
          )}
  ...
  // --- TELA PRINCIPAL (LISTA DE SENHAS) ---
  return (
    <div style={{ width: '100%' }}>
      {toast && (
        <div className={`toast ${toast.type}`}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
        <div>
             <h2 style={{margin: 0, fontSize: '1.8rem'}}>
                 Cofre de <span style={{color: 'var(--accent)'}}>Contas</span>
             </h2>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
            <button 
                onClick={onBack} 
                style={{
                    background: 'transparent', border: '1px solid var(--border)', color: '#94a3b8', padding: '8px 12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.9rem', transition: 'all 0.2s ease'
                }}
            ...
            <button 
                onClick={() => { setIsUnlocked(false); setMasterPassword(''); setShowForm(false); }} 
      ...
      {!showForm ? (
        <button onClick={() => setShowForm(true)} className="btn-add-placeholder">
          <Plus size={24} /> Adicionar Nova Conta
        </button>
      ) : (
        <div className="card" style={{ borderColor: editingId ? '#10b981' : '#334155' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: 10, color: editingId ? '#10b981' : 'white', margin: 0 }}>
              ...
              {editingId ? 'Editar Conta' : 'Adicionar Nova'}
            </h3>
          </div>
          ...
          <form onSubmit={handleSave}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '10px' }}>
              ...
              <div>
                <PasswordInput placeholder="Senha" value={newPass} onChange={(e: any) => setNewPass(e.target.value)} show={showNewPass} onToggle={() => setShowNewPass(!showNewPass)} onGenerate={generateStrongPassword} />
                
                {newPass && strength && (
                  <div className="strength-meter-container">
                    <div className={`strength-meter-fill strength-${strength}`}></div>
                  </div>
                )}
                {newPass && strength && (
            ...
            <div className="form-actions">
              <button type="submit" className="btn-primary" disabled={loading} style={{ backgroundColor: editingId ? '#10b981' : '' }}>
                {loading ? '...' : (editingId ? 'Atualizar' : 'Salvar')}
              </button>
              <button type="button" onClick={handleCloseForm} className="btn-danger">
                Cancelar
              </button>
            </div>
          </form>
      ...
      <div className="vault-grid">
        {filteredItems.map((item) => {
          const decryptedUser = decryptData(item.username_encrypted, masterPassword);
          const decryptedPass = decryptData(item.password_encrypted, masterPassword);
            ...
            <div key={item.id} className="vault-item" style={{ borderColor: isEditing ? '#10b981' : '#334155', opacity: (editingId && !isEditing) ? 0.5 : 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                <div style={{display: 'flex', alignItems: 'center', gap: '12px', overflow: 'hidden'}}>
                  <img 
                    src={getFaviconUrl(item.site_name)} 
                    alt="icon"
                    style={{width: '32px', height: '32px', borderRadius: '50%', objectFit: 'cover', backgroundColor: 'white'}}
                    onError={(e) => { e.currentTarget.style.display = 'none'; }} 
                  />
                  <strong style={{ fontSize: '1.2em', color: isEditing ? '#10b981' : 'white', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.site_name}
                  </strong>
                </div>
              ...
              {decryptedUser ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div style={{ background: '#020617', padding: '10px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'monospace', color: isPassVisible ? '#a78bfa' : '#64748b', fontSize: '0.9em' }}>
                      {isPassVisible ? decryptedPass : '••••••••'}
                    </span>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <button onClick={() => setVisiblePasswordId(isPassVisible ? null : item.id)} style={{ background: 'transparent', padding: 0, color: '#94a3b8' }}>
